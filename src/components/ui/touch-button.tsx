  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        ...
        icon: "h-10 w-10",
        touch: "h-12 px-6 py-3", // Enhanced touch target
        "touch-lg": "h-14 px-8 py-4", // Large touch target
      },
    },
    defaultVariants: {
      variant: "default",
...
const TouchButton = React.forwardRef<HTMLButtonElement, TouchButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    // Automatically use touch-optimized sizes on touch devices
    const touchOptimizedSize = isTouchDevice && size === "default" ? "touch" : size

    return (
      <Comp
        className={cn(
          touchButtonVariants({ variant, size: touchOptimizedSize, className }),
          isTouchDevice && "transform-gpu will-change-transform"
        )}
        ref={ref}
        {...props}
      />
