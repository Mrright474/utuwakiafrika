  ...
  return (
    <div ref={imgRef} className={cn("relative overflow-hidden", className)}>
      ...
      {isInView && !isLoaded && !hasError && (
        <div className="absolute inset-0 flex items-center justify-center bg-utu-cream/20">
          <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-utu-red"></div>
        </div>
      )}
    </div>
  );
